...
30-minute granularity, hard deadlines, and a finite user-selected working window. The
implementation uses a CP-SAT model when OR-Tools is available and falls back to a deterministic
greedy heuristic when it is not.
...
H = \frac{b-a}{30}
\]
half-hour slots corresponding to the active workday window \([a,b)\), where \(a\) and \(b\) are
times of day measured in minutes from midnight and must align to the 30-minute grid. The
default window is \(09{:}00\) to \(17{:}00\), but the UI can override that per planning run.
The compressed weekly
slot set is
\[
S = \{0, \dots, 5H - 1\}.
\]
...
\begin{itemize}
  ...
  \item priority \(w_i \in \{1,2,3\}\), where \(1\) means ``when possible,'' \(2\) means ``important,'' and \(3\) means ``urgent,''
...
Only the portion of an event that overlaps the selected workday window contributes to the busy
interval set. Those intervals are immutable and remove capacity from the feasible region.
...
Here \(\delta_i\) is the due date mapped onto the compressed weekly slot axis and clipped to the
selected workday window for the corresponding day.
...
\begin{itemize}
  \item schedule urgent work ahead of important work, and important work ahead of work that is merely ``when possible,''
...
\begin{enumerate}
  \item descending priority level: urgent, then important, then when possible,
...
\begin{itemize}
  ...
  \item The workday window applies to one planning run; it is not yet stored as a persistent user preference.
  \item Context switching, travel, and preferred sequencing are not modelled yet.
  \item Priority is still treated as a scalar three-level weight rather than a richer utility curve.
\end{itemize}
...
\begin{itemize}
  ...
  \item scheduled task blocks placed by the optimiser and coloured by the three priority levels,

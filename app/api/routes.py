...
def demo_page(request: Request, db: DbSession) -> HTMLResponse:
    ...
    return templates.TemplateResponse(
        ...
        context={
            ...
            "routes": {
                "create_task": str(request.url_for("create_task")),
                "create_event": str(request.url_for("create_event")),
                "reset_demo": str(request.url_for("reset_demo")),
                "generate_schedule": str(request.url_for("generate_schedule")),
                "export_calendar_ics": str(request.url_for("export_calendar_ics")),
                "delete_task_template": str(request.url_for("delete_task", task_id=0)),
                "delete_event_template": str(request.url_for("delete_event", event_id=0)),
            },
        },
    )
